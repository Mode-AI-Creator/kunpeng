...
3. 深度理解剧本后，调用 workshop_set_breakdown 一次性写入：
   ...
   - 剧本中的车辆只要参与事件、展示或是重要道具，就必须按 car-commercial-storyboard 技能「任务一」建档：正前/正侧/正后三视图与轮毂/灯组/车标/传感器等细节图各自独立成资产（命名如「车型-正侧」「车型-轮毂」），用户给了实车参考图时以完全复刻为准；不能只在场景描述里把车辆一笔带过。车辆的结构不可漂移项（车身比例、灯组、格栅、轮毂、颜色、标识）写进 bibles
...
5. 再调用 workshop_set_shots 写入分镜表（每镜：shotNo 用"集-镜"如 01-01、sourceFactIds、narrativeFunction、画面描述、对白、景别、运镜、情绪、时长 8-15 秒（Seedance 2.5 项目可到 30 秒）、关联 characterIds/sceneId，以及仅供 Agent 使用的 directorDecision）。**长剧本分批写，每批 ≤15 镜，第二批起用 mode:"merge"**
   ...
   - 车辆参与的分镜按 car-commercial-storyboard 技能「任务三」执行：全片锁定行驶方向，越轴必须有动机，相邻镜头速度感连续，车漆反射随环境变化，开门/落座/点火/起步等动作按因果链给接点；TVC 展示片按技能「任务二」的段落结构排镜（开场空镜→路跑→单主体细节→内饰→落点）
...
【GPT-Image-2.5】（默认引擎，GPT 生图 = 图像模型 gpt-image-2.5；勿与视频模型 Seedance 2.5 混淆）→ **中文段落式**，参考结构：
   ...
   - **车辆资产（复刻纪律，最高优先级）**：有实车参考图时必须写「完全复刻 @图片N 中的同一辆车：保持车身比例、线条、灯组、格栅、轮毂、车漆与标识完全一致」，禁止泛化车型描述、禁止重新设计、禁止混入其他车型语言。正前/正侧/正后三视图各自一条独立资产提示词（正交视角、无缝白/浅灰摄影棚、均匀柔光、车身完整入画、机位高度一致），轮毂/灯组/车标/传感器等细节图同样各自独立——禁止把多个视图拼进一张组合图
