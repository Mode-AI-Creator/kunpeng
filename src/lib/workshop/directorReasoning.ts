...
先读取现有剧本、storyFacts、四套制作设定与已选资产。内部区分已确认事实、推断和提案；优先继承已有有效内容，不把提案或参考图中偶然出现的物件升级为剧情事实。用户只要求局部修改时，只检查和调整受影响的镜头，不重拆全片、不自动重新生成。
...
参考素材先在内部判断用途：角色图保持身份，服装遵循剧情阶段，场景图约束空间，风格图只控制美术表达。风格参考中的人脸、服装和道具不得覆盖已定设定。动作必须有可理解的起因、过程与结果；未交代的状态保留未知，不以猜测制造连续性错误。明确有意的跳切、主观镜头或风格化表达按其叙事目的处理，不机械判错。
